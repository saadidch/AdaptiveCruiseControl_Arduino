 ...
 * @file LCD.h
 *
 * Class definition for LCD that wraps APIs of LiquidCrystal library
 *
 * @copyright Copyright 2015-2016 The MathWorks, Inc.
 *
 */

// Inherit from the LibraryBase class and LiquidCrystal header file
...
public:
    ...
    {
        libName = "ExampleLCD/LCDAddon";
        a.registerLibrary(this);
    }
    
    // reset cursor's position to the first row
    void setup()
    {
        cursorRow = 0;
    }
...
public:
    void commandHandler(byte cmdID, byte* dataIn, unsigned int payloadSize)
    {
        switch(cmdID)
        {
            ...
            {
                unsigned int cols = dataIn[0];
                unsigned int rows = dataIn[1];
                initializeLCD(cols,rows);
                clearLCD();
                sendResponseMsg(cmdID, 0, 0);
                break;
            }
...
// Wrap LiquidCrystal methods to add debug messages
public:
